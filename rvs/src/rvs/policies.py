...
from rvs.src.rvs import dataset, layers, step, util
from stable_baselines3.common.distributions import SquashedDiagGaussianDistribution, Distribution
...
def make_obs_goal_space(
    ...
    if unconditional_policy:
        ...
        print("reward conditioning, rtg dim: ")
        print(rtg_dim)
        return util.add_scalar_to_space(observation_space, rtg_dim)
...
class RvS(pl.LightningModule):
    ...
    def training_step(
        ...
        loss = torch.mean((action - prediction) ** 2)
        if loss < 0:
            print("wrong")
        # loss = -log_probs.mean()
...
class ExtendedActorCriticPolicy(policies.ActorCriticPolicy):
    ...
    def _get_action_dist_from_latent(self, latent_pi: torch.Tensor, latent_sde: Optional[torch.Tensor] = None) -> Distribution:

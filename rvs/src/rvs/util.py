...
from rvs.src.rvs import step, train
...
def add_scalar_to_space(
    observation_space: Union[spaces.Box, spaces.Dict],
    n_obj: int = 1 # default adding 1 time
) -> spaces.Box:
    ...
    lows = np.concatenate([observation_space.low, [-np.inf for _ in range(n_obj)]])
    highs = np.concatenate([observation_space.high, [np.inf for _ in range(n_obj)]])
